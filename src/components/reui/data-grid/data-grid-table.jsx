    ...
    return (
      <tr>
        <td colSpan={totalColumns} className="p-0">
          {props.emptyState({ table })}
        </td>
      </tr>
    );
  }

  const title = props.isError
    ? props.errorTitle || "Ma'lumot yuklanmadi"
    : props.emptyTitle || "Ma'lumot topilmadi";
  const message = props.isError
    ? props.errorMessage || "Server bilan bog'lanishda xatolik yuz berdi."
    : props.emptyMessage ||
      "Filterlarni o'zgartiring yoki yangi yozuv yarating.";
    ...
    <tr>
      ...
      >
        <div className="mx-auto flex max-w-sm flex-col items-center gap-1.5">
          ...
          {props.isError && typeof props.onRefresh === "function" ? (
            <button
              type="button"
              className="mt-2 rounded-xl border px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-muted"
