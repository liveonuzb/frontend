...
const Index = ({ className, ...props }) => {
  ...
  const { mutateAsync: loginGoogle, isPending: isGooglePending } = usePostQuery({
    mutationProps: {
      onSuccess: (response) => {
      ...
      onError: (error) => {
        toast.error(getAuthErrorMessage(error, "Google bilan kirishda xatolik yuz berdi."));
      },
    },
  });
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      // access_token is received from implicit flow, which we send to backend
      await loginGoogle({
        url: "/auth/login/google",
        attributes: { token: tokenResponse.access_token },
      });
    },
    onError: () => toast.error("Google bilan boglanish amalga oshmadi"),
  });

  const clearPasswordReset = useAuthStore((state) =>
    get(state, "clearPasswordReset"),
  );
