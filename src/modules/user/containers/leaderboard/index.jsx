...
const LeaderboardPage = () => {
  ...
  const { data: leaderboardData, isLoading } = useGetQuery({
    url: "/user/gamification/leaderboard",
    params: { period, scope },
    queryProps: {
      queryKey: ["user", "gamification", "leaderboard", period, scope],
    },
  });
  ...
  const { data: xpHistoryData } = useGetQuery({
    ...
    params: { limit: 10 },
    queryProps: { queryKey: ["user", "gamification", "xp-history"] },
  });
  ...
  React.useEffect(() => {
    setBreadcrumbs([
      ...
      { url: "/user/leaderboard", title: "Reyting" },
    ]);
  }, [setBreadcrumbs]);
