...
const EXAMPLE_PROMPTS = [
  ...
  "200 ml sut va 3 ta pechenye",
  "150 g tovuq va 100 g guruch",
  "1 olma va 1 stakan yogurt",
];
...
const TextAddDrawer = ({
  ...
  const isAudioTranscript = variant === "audio";
  const title = isAudioTranscript ? "Transcriptni tekshirish" : "Matn orqali qo'shish";
  const description = isAudioTranscript
    ? "Audio matnga aylantirildi. Xohlasangiz tahrir qiling va keyin yuboring."
    : "Ovqatni matn bilan yozing. Masalan: 2 ta tuxum, 1 ta banan, 200 ml sut.";
  const helperText = isAudioTranscript
    ? "Yuborgandan keyin AI matnni ovqatlarga ajratadi, siz porsiyani tekshirasiz."
    : "Keyingi bosqichda AI ovqatlarni topadi, siz porsiyani tekshirib log qilasiz.";
  const continueLabel = isAudioTranscript ? "Yuborish" : "Davom etish";
  const confidenceMeta =
    isAudioTranscript && typeof transcriptConfidence === "number"
  ...
  return (
    <>
      ...
      <DrawerBody className="space-y-4">
              ...
              <div className="mt-4 rounded-2xl border px-3 py-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className={`mt-1 text-sm font-semibold ${confidenceMeta.tone}`}>
        ...
        />
        {isAudioTranscript && transcriptSegments.length > 0 ? (
        ...
        {isAudioTranscript && transcriptHistory.length > 0 ? (
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-3">
      ...
      <DrawerFooter>
        {isAudioTranscript && onAppendAudio ? (
          <Button variant="outline" onClick={onAppendAudio} disabled={isSubmitting}>
            <MicIcon className="size-4" />
            Yana audio qo'shish
          </Button>
        ) : null}
        <Button onClick={onContinue} disabled={!value.trim() || isSubmitting}>
          {continueLabel}
        </Button>
