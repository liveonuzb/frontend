...
import { getIngredientNutritionSourceLabel } from "../recipe-builder-payload.js";
...
const scaleValue = (ingredient, key) => {
  const baseQuantity = Math.max(
    0.01,
    toNumber(ingredient.baseQuantity || ingredient.quantity || 1),
  );
...
const IngredientRow = ({ ingredient, onChange, onDelete }) => {
  const sourceLabel = getIngredientNutritionSourceLabel(ingredient);
  const hasSourceIssue =
    sourceLabel === "Tekshirish kerak" || sourceLabel === "Noma'lum";
  ...
  return (
    <TableRow>
      <TableCell className="min-w-56">
        <div className="flex items-center gap-3">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            ...
            >
              <SelectTrigger
                aria-label={`${ingredient.name} mahsulot`}
                className="w-full"
              >
                <SelectValue />
              </SelectTrigger>
            ...
            <Badge variant={hasSourceIssue ? "destructive" : "secondary"}>
              {sourceLabel}
            </Badge>
          </div>
        </div>
      ...
      <TableCell className="min-w-28">
        ...
        >
          <SelectTrigger
            aria-label={`${ingredient.name} birligi`}
            className="w-full"
          >
            <SelectValue />
          </SelectTrigger>
      ...
      <TableCell className="min-w-44">
        <Input
          ...
          onChange={(event) =>
            onChange(ingredient.id, { note: event.target.value })
          }
        />
      </TableCell>
      <TableCell className="min-w-44">
