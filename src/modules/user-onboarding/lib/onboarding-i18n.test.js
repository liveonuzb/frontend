...
const flattenKeys = (value, prefix = "") => {
  ...
  return flatMap(toPairs(value), ([key, child]) =>
    flattenKeys(child, prefix ? `${prefix}.${key}` : key),
  );
};

const listSourceFiles = (directory) =>
  flatMap(fs.readdirSync(directory, { withFileTypes: true }), (entry) => {
...
const hardcodedOnboardingCopy = [
  "Onboarding yakuniy bosqichi",
  "Yakuniy",
  "Metabolizm hisobingiz tayyor",
  "AI tahlili",
...
describe("onboarding i18n", () => {
  ...
  it("does not leave known onboarding result/header copy hardcoded in source", () => {
    const findings = [];

    forEach(listSourceFiles(onboardingRoot), (filePath) => {
      const source = fs.readFileSync(filePath, "utf8");

      forEach(hardcodedOnboardingCopy, (copy) => {
        if (includes(source, copy)) {
          findings.push(`${path.relative(onboardingRoot, filePath)}: ${copy}`);
        }
      });
    });

    expect(join(findings, "\n")).toBe("");
  });
});
