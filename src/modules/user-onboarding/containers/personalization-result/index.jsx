...
const editableFieldMeta = {
  [editKeys.dailyCalories]: {
    icon: FlameIcon,
    labelKey: "dailyCalories",
    unit: "kcal",
    step: 25,
    min: 800,
    formatOptions: { maximumFractionDigits: 0 },
    helperKey: "dailyCalories",
  },
  [editKeys.proteinGram]: {
    icon: SaladIcon,
    labelKey: "proteinGram",
    unit: "g",
    step: 5,
    min: 0,
    formatOptions: { maximumFractionDigits: 0 },
    helperKey: "proteinGram",
  },
  [editKeys.carbsGram]: {
    icon: UtensilsIcon,
    labelKey: "carbsGram",
  ...
  [editKeys.recommendedWaterMl]: {
    icon: DropletsIcon,
    labelKey: "recommendedWaterMl",
    unit: "L",
    step: 0.1,
    min: 0.5,
    formatOptions: { maximumFractionDigits: 1 },
    helperKey: "recommendedWaterMl",
    water: true,
  },
};

const getEditableFieldMeta = (field, t) => {
  const meta = editableFieldMeta[field];
  if (!meta) return null;

  return {
    ...meta,
    label: t(`onboarding.postOnboarding.result.edit.${meta.labelKey}.label`),
    helper: t(`onboarding.postOnboarding.result.edit.${meta.helperKey}.helper`),
  };
};
...
const EditBadgeButton = ({ editLabel, onClick, className }) => (
  <button
    type="button"
    onClick={onClick}
...
const getCalculationBadge = (item, t) => {
  if (item.key === "bmr") {
    return t("onboarding.postOnboarding.result.calculation.badges.base");
  }
  if (item.key === "activity") return "x";
  if (item.key === "tdee") return "=";
  if (item.key === "adjustment") {
    if (startsWith(String(item.value), "-")) {
      return t("onboarding.postOnboarding.result.calculation.badges.deficit");
    }
    if (startsWith(String(item.value), "+")) {
      return t("onboarding.postOnboarding.result.calculation.badges.surplus");
    }
    return "0";
  }
  if (item.key === "final") {
    return t("onboarding.postOnboarding.result.calculation.badges.target");
  }
  return item.operator;
};

const CalculationSummary = ({ steps, t }) => {
        ...
        {map(summarySteps, (item, index) => (
          <React.Fragment key={item.key}>
            ...
            >
              ...
              >
                {item.key === "final"
                  ? t("onboarding.postOnboarding.result.calculation.finalLabel")
                  : item.label}
              </span>
...
const CalculationStep = ({ item, index, isLast, t }) => {
  const Icon = metricIconMap[item.key] ?? TargetIcon;
  const badge = getCalculationBadge(item, t);
...
const EditDrawer = ({
  ...
  const editMeta = getEditableFieldMeta(field, t);
            ...
            <div className="rounded-[1.25rem] border border-[#ff99002e] bg-[rgba(30,20,11,0.72)] p-4">
              <div className="flex items-center justify-between gap-3">
                <span className="text-[11px] font-black uppercase tracking-[0.14em] text-white/45">
                  {t("onboarding.postOnboarding.result.currentValue")}
                </span>
          ...
          {editMeta ? (
            <div className="space-y-2">
              <label className="text-[13px] font-black text-white">
                {t("onboarding.postOnboarding.result.newValue")}
              </label>
              <NumberField
...
export const ResultContent = ({ result, onboarding, onEdit }) => {
  ...
  const snapshot = React.useMemo(
    () => buildMetabolismResultViewModel(result, onboarding, t),
    [onboarding, result, t],
  );
  const macroItems = [
    {
      ...
      editLabel: t("onboarding.postOnboarding.result.editButtonAria", {
        label: snapshot.macros.protein.label,
      }),
    },
    {
      key: "carbs",
      ...
      editLabel: t("onboarding.postOnboarding.result.editButtonAria", {
        label: snapshot.macros.carbs.label,
      }),
    },
    {
      key: "fat",
      ...
      editLabel: t("onboarding.postOnboarding.result.editButtonAria", {
        label: snapshot.macros.fat.label,
      }),
    },
    {
      key: "water",
      label: t("onboarding.postOnboarding.result.water"),
  ...
  return (
    <div className="relative mx-auto flex h-full min-h-0 w-full max-w-[430px] flex-1 flex-col gap-4 overflow-y-auto overflow-x-hidden bg-[#070503] px-3 pb-20 pt-3 text-white min-[390px]:px-4">
      ...
      >
        ...
        <div className="relative max-w-[520px]">
          ...
          <h1 className="mt-4 max-w-[240px] text-[1.7rem] font-black leading-[1.06] tracking-tight text-white min-[390px]:text-[1.85rem]">
            <span className="sr-only">{snapshot.title}</span>
            <span aria-hidden="true">
              {t("onboarding.postOnboarding.result.heroTitleLine1")}
              <br />
              {t("onboarding.postOnboarding.result.heroTitleLine2")}
            </span>
          </h1>
          <p
            className={cn(
        ...
        <IconBubble icon={BrainIcon} className="size-12" glow />
          <span className="min-w-0 flex-1">
            <span className="block text-[15px] font-black text-[#ffb000]">
              {t("onboarding.postOnboarding.result.aiAnalysisTitle")}
            </span>
          <span
            className={cn(
      ...
      <SectionCard
        title={t("onboarding.postOnboarding.result.dailyCaloriesSectionTitle")}
        className="p-4"
      >
        <div className="grid gap-4">
          <div className="relative flex min-h-[168px] flex-col justify-center rounded-[1.2rem] pr-10 text-left">
            <EditBadgeButton
              editLabel={t("onboarding.postOnboarding.result.editButtonAria", {
                label: t(
                  "onboarding.postOnboarding.result.dailyCaloriesSectionTitle",
                ),
              })}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                >
                  {t("onboarding.postOnboarding.result.dailyTarget")}
                </span>
      ...
      <SectionCard title={t("onboarding.postOnboarding.result.dailyIndicatorsTitle")}>
        <div className="grid grid-cols-2 gap-2.5">
      ...
      <SectionCard title={t("onboarding.postOnboarding.result.calculationTitle")}>
        <p
          className={cn(
        ...
        >
          {t("onboarding.postOnboarding.result.calculationDescription")}
        </p>
        <CalculationSummary steps={snapshot.calculationSteps} t={t} />
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1.5 text-[11px] font-black text-[#ffb000]">
            ...
            {t("onboarding.postOnboarding.result.formulaLabel", {
