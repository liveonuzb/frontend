...
const Index = () => {
  ...
  const { options: apiGoals } = useOnboardingOptions({
    resource: "goals",
    step: "goal",
  });
  ...
  const goals = React.useMemo(() => {
    const source = apiGoals.length ? apiGoals : defaultGoals;
    const weightGoals = filter(
      source,
      (item) => (item.goalType || "weight") === "weight",
    );
    return map(
      weightGoals.length ? weightGoals : defaultGoals,
      (item, index) => {
        const tone =
          toneMap[item.key] ??
