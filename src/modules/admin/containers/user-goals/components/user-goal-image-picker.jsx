...
const UserGoalImagePicker = ({ value, onChange, onUploadingChange }) => {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      formData.append("folder", "user-goals");
      ...
      if (!uploadedUrl) throw new Error("Upload response does not contain URL");
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between gap-3">
        <Label className="text-xs font-bold">Rasm</Label>
        {value ? (
          <Button
            type="button"
            variant="ghost"
      ...
      >
        {value ? (
          <img src={value} alt="Maqsad rasmi" className="size-full object-cover" />
        ) : (
