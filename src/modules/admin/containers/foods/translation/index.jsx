...
const buildPayload = (values, languages, currentLanguage) => {
  const translations = reduce(languages, (acc, language) => {
    const code = get(language, "code");
...
const TranslateFood = () => {
  ...
  const { data: foodData, isLoading } = useGetQuery({
    url: `/admin/foods/${id}`,
    queryProps: {
  ...
  const item = getResponsePayload(foodData);
  ...
  const handleOpenChange = (open) => {
    if (!open) navigate("/admin/foods/list");
  };
        ...
        <div className="mx-auto flex min-h-0 w-full flex-1 flex-col">
          <DrawerHeader className="items-center text-center">
            ...
            <DrawerDescription className="max-w-sm">
              {item
                ? `${trim(get(item, "name", "")) || "Ovqat"} uchun faol tillardagi nomlar`
                : "Ovqat tarjimalari"}
            </DrawerDescription>
          </DrawerHeader>
