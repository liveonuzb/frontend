...
const createFormFromWorkout = (workout, language) => {
  const ensureAtLeastOne = (arr) =>
    isArray(arr) && arr.length > 0 ? arr : [""];
  const ensureArray = (arr) =>
    isArray(arr)
      ? lodashMap(chain(arr), (item) => trim(String(item)))
          .compact()
          .value()
      : [];

  return {
    name: resolveLabel(workout?.translations, workout?.name ?? "", language),
    isActive: workout?.isActive ?? true,
    isOnboarding: workout?.isOnboarding ?? true,
    trackingType: workout?.trackingType ?? WORKOUT_TRACKING_TYPES.REPS_WEIGHT,
    defaultSets: String(workout?.defaultSets ?? 3),
    defaultReps:
      workout?.defaultReps !== null && workout?.defaultReps !== undefined
        ? String(workout.defaultReps)
        : "",
    defaultDurationSeconds:
      workout?.defaultDurationSeconds !== null &&
      workout?.defaultDurationSeconds !== undefined
        ? String(workout.defaultDurationSeconds)
        : "",
    defaultDistanceMeters:
      workout?.defaultDistanceMeters !== null &&
      workout?.defaultDistanceMeters !== undefined
        ? String(workout.defaultDistanceMeters)
        : "",
    defaultRestSeconds: String(workout?.defaultRestSeconds ?? 60),
    categoryIds: isArray(workout?.categoryIds) ? workout.categoryIds : [],
    youtubeUrl: get(workout, "youtubeUrl", ""),
    targetMuscles: ensureArray(get(workout, "targetMuscles", [])),
    bodyParts: ensureArray(get(workout, "bodyParts", [])),
    equipments: ensureArray(get(workout, "equipments", [])),
    secondaryMuscles: ensureArray(get(workout, "secondaryMuscles", [])),
    instructions: ensureAtLeastOne(get(workout, "instructions", [])),
    imageFile: null,
    removeImage: false,
    equipmentImageFiles: [],
    existingEquipmentImages: isArray(workout?.equipmentImages)
      ? workout.equipmentImages
      : [],
    removeEquipmentImages: false,
    translations: isObject(workout?.translations) ? workout.translations : {},
  };
};
...
const EditWorkoutPage = () => {
  ...
  const editingWorkout =
    get(workoutData, "data.data", null) || get(workoutData, "data", null);
  ...
  React.useEffect(() => {
    if (editingWorkout && !formInitialized) {
      setForm(createFormFromWorkout(editingWorkout, currentLanguage));
      setFormInitialized(true);
    }
  }, [editingWorkout, currentLanguage, formInitialized]);
    ...
    try {
      ...
      for (const file of form.equipmentImageFiles) {
        if (file) {
          ...
          if (fileId) equipmentImageIds.push(fileId);
        }
      }
        ...
        <DrawerBody className="space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-start gap-4">
              <ImageUploadPreview
                file={form.imageFile}
                existingUrl={editingWorkout?.imageUrl}
