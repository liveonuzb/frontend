...
import { useMealPlanFilters } from "./use-filters.js";
...
const MealPlansListPage = () => {
  ...
  const { data: plansData, isLoading, isFetching, refetch } = useCoachMealPlans(queryParams);
  ...
  const handleSoftDelete = async () => {
    ...
    try {
      ...
      toast.success("Reja trashga yuborildi");
    ...
    try {
      ...
      toast.success(
        hardDeleteTarget.ids.length === 1
          ? "Reja butunlay o'chirildi"
          : `${hardDeleteTarget.ids.length} ta reja butunlay o'chirildi`,
      );
  ...
  const handleBulkTrash = async () => {
    ...
    try {
      ...
      toast.success(`${selectedPlanIds.length} ta reja trashga yuborildi`);
      setRowSelection({});
    } catch (error) {
      const message = error?.response?.data?.message;
      toast.error(
  ...
  const handleBulkRestore = async () => {
    ...
    try {
      ...
      toast.success(`${selectedPlanIds.length} ta reja tiklandi`);
      setRowSelection({});
    } catch (error) {
      const message = error?.response?.data?.message;
      toast.error(
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      <ListHeader
        title="Ovqatlanish rejalari"
        description="Trener ovqatlanish rejalarini boshqaring"
        actions={[
          {
            key: "create",
            label: "Yangi reja",
