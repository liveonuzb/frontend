...
import {
  useCoachSnippets,
  useCoachSnippetsMutations,
} from "@/modules/coach/lib/hooks/useCoachSnippets";
import { useSnippetFilters } from "../list/use-filters.js";
import SnippetsListPage from "../list/index.jsx";
...
vi.mock("@/components/reui/data-grid", () => ({
  DataGrid: ({ children, recordCount }) => (
    <div data-record-count={recordCount} data-testid="snippets-data-grid">
      {children}
    </div>
  ),
  DataGridContainer: ({ children }) => <div>{children}</div>,
  DataGridTable: () => <div data-testid="snippets-table">table</div>,
  DataGridTableDndRows: ({ handleDragEnd }) => (
    <button
      type="button"
      onClick={() =>
        handleDragEnd({ active: { id: "one" }, over: { id: "two" } })
      }
    >
      Drag reorder
    </button>
...
vi.mock("@/components/reui/data-grid/data-grid-pagination", () => ({
  DataGridPagination: () => <div data-testid="snippets-pagination" />,
}));
...
vi.mock("../list/filter.jsx", () => ({
  Filter: ({ activeFilters }) => (
    <div data-testid="snippets-filter">{activeFilters.length}</div>
  ),
}));
...
vi.mock("../list/use-filters.js", () => ({
  useSnippetFilters: vi.fn(),
}));

vi.mock("@/modules/coach/lib/hooks/useCoachSnippets", () => ({
  useCoachSnippets: vi.fn(),
  useCoachSnippetsMutations: vi.fn(),
}));
...
const setupFilters = (overrides = {}) => {
  vi.mocked(useSnippetFilters).mockReturnValue({
    search: "salom",
    lifecycle: "active",
    setLifecycle,
    sortBy: "updatedAt",
    ...
    filterFields: [],
    activeFilters: [
      { id: "q", field: "q", operator: "contains", values: ["salom"] },
    ],
...
const setupHooks = () => {
  vi.mocked(useCoachSnippets).mockReturnValue({
    data: {
      data: {
        data: [
          { id: "one", title: "Salom", isDefault: false },
          { id: "two", title: "Follow up", isDefault: false },
        ],
  ...
  vi.mocked(useCoachSnippetsMutations).mockReturnValue({
...
describe("SnippetsListPage", () => {
  ...
  it("queries snippets with URL filters and resets page on lifecycle change", () => {
    setupFilters();
    setupHooks();

    render(
      <MemoryRouter initialEntries={["/coach/snippets/list"]}>
        <SnippetsListPage />
      </MemoryRouter>,
    );

    expect(screen.getByText("Shablonlar")).toBeInTheDocument();
    expect(screen.getByTestId("snippets-filter")).toHaveTextContent("1");
    expect(useCoachSnippets).toHaveBeenCalledWith({
      q: "salom",
      lifecycle: "active",
      sortBy: "updatedAt",
      sortDir: "desc",
